...
public class BanzaiApplication implements Application {
    ...
    public void send44(Order order) {
        ...
        //String account="973737";
        //switch(iCtr++)
        //{
        //    case 1://Place a market order with a qty=2 
                ...
                //newOrderSingle.setField(new StringField(40,"6"));//For OrderType=6 test case
          ...
          //      break;
            /*case 2: // Cancel
                ...
                quickfix.fix44.OrderCancelRequest message = new quickfix.fix44.OrderCancelRequest(
                        new OrigClOrdID(ordId), new ClOrdID(id),
                ...
                message.setField(new Account(account));
                send(message, order.getSessionID());
                break;                
            case 3://Place an order with qty=2, then modify qty=8, and modify qty=4
                    ordId = order.getID();
                    quickfix.fix44.NewOrderSingle newOrderSingleLimit = new quickfix.fix44.NewOrderSingle(
                        new ClOrdID(order.getID()), sideToFIXSide(order.getSide()),
                        new TransactTime(), typeToFIXType(order.getType()));
                ...
                //newOrderSingle.setField(new StringField(100,"XNYS"));
                send(populateOrder(order, newOrderSingleLimit), order.getSessionID());
                break;
            case 4: // MODIFY to 8 for rejection
                Order newOrder = orderTableModel.getOrder(message.getField(new ClOrdID()).getValue());
                String id2 = newOrder.generateID();
                quickfix.fix44.OrderCancelReplaceRequest message2 = new quickfix.fix44.OrderCancelReplaceRequest(
                        new OrigClOrdID(ordId), new ClOrdID(id2),
                        sideToFIXSide(order.getSide()), new TransactTime(), typeToFIXType(order.getType()));
    ...
    public void cancel42(Order order) {
      ...
      public void cancel44(Order order) {
        String id = order.generateID();
        String account="845672";
        quickfix.fix44.OrderCancelRequest message = new quickfix.fix44.OrderCancelRequest(
                new OrigClOrdID(order.getID()), new ClOrdID(id),
                sideToFIXSide(order.getSide()), new TransactTime());
        message.setField(new OrderQty(order.getQuantity()));
        message.setField(new Symbol(order.getSymbol()));
//        message.setField(new SecurityID ("US0605051046"));
//        message.setField(new SecurityIDSource ("4"));
//        message.setField(new HandlInst('1'));
        message.setField(new Account(account));
//        message.setField(new ExDestination("XNYS"));
//        message.setField(new StringField(15,"USD"));
    ...
    public void replace44(Order order, Order newOrder) {
        quickfix.fix44.OrderCancelReplaceRequest message = new quickfix.fix44.OrderCancelReplaceRequest(
                new OrigClOrdID(ordId), new ClOrdID(newOrder.getID()),
                sideToFIXSide(order.getSide()), new TransactTime(),
                typeToFIXType(order.getType()));
                message.set(new OrderQty(order.getQuantity()));
                message.set(new Symbol(order.getSymbol()));
                message.set(new SecurityID ("US0605051046"));
                message.set(new SecurityIDSource ("4"));
                message.set(new Account("845673"));
                message.set(new ExDestination("XNYS"));
                message.setField(new StringField(15,"USD"));
